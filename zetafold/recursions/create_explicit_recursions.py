...
for n,line in enumerate(lines):
    ...
    if len( line ) > 1 and line[0] != ' ' or n == len(lines)-1:
        if len( lines_standard ) > 0:
            lines_new.append('    if not self.options.use_logsumexp: # AUTOGENERATED LOGSUMEXP BLOCK\n')
            lines_new     += lines_standard
            lines_new += '\n'
            lines_standard = []

            lines_new.append('    else: # AUTOGENERATED LOGSUMEXP BLOCK\n')
            lines_new     += lines_logsumexp
    ...
    if first_char != '#' and line_new.count( 'def' ) == 0 and not is_assignment_line and \
       not in_comment_block and not line.count( "'''" ) and first_char != '' and num_indent >= 4:
        ...
        lines_logsumexp.append( ' '*4 + line_new )
    ...
    if assign_pos > -1:
        ...
        if len( Qpos ) > 0 and Qpos[0] < assign_pos:
            ...
            line_logsumexp += ' '*4
            line_logsumexp += 'contribs.append( '
            line_logsumexp += line_new[assign_pos+3:-1]
            line_logsumexp += ')\n'
